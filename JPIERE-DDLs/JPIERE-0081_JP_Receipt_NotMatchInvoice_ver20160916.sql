-- DROP VIEW adempiere.JP_Receipt_NotMatchInvoice;
CREATE OR REPLACE VIEW adempiere.JP_Receipt_NotMatchInvoice AS
SELECT 
...
FROM  adempiere.M_InOut io   
	...
	INNER JOIN adempiere.C_DocType dt ON(io.C_DocType_ID=dt.C_DocType_ID)
	 LEFT OUTER JOIN (SELECT M_InOutLine_ID, SUM(COALESCE(Qty,0)) AS Qty From adempiere.M_MatchInv GROUP BY M_InOutLine_ID) m ON (m.M_InOutLine_ID=iol.M_InOutLine_ID)
